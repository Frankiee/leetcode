...
# there is no flight from the city i to the city j, flights[i][j] = 0; Otherwise, flights[i][j] =
# 1. Also, flights[i][i] = 0 for all i.
# You totally have K weeks (each week has 7 days) to travel. You can only take flights at most
# once per day and can only take flights on each week's Monday morning. Since flight time is so
# short, we don't consider the impact of flight time.
...
class Solution(object):
    def maxVacationDays(self, flights, days):
        ...
        :rtype: int
        """
        dp = [[float('-inf')] * len(days[0]) for _ in range(len(flights[0]))]

        for w in range(len(days[0])):
            for c in range(len(flights[0])):
                if w == 0:
                    if c == 0:
                        dp[c][w] = days[0][w]
                    if flights[0][c] == 1:
                        dp[c][w] = max(days[c][w], dp[c][w])
                else:
                    dp[c][w] = dp[c][w - 1] + days[c][w]
                    for p_c in range(len(flights[0])):
                        if flights[p_c][c] == 1:
                            dp[c][w] = max(dp[c][w], dp[p_c][w - 1] + days[c][w])

        return max([dp[c][-1] for c in range(len(flights[0]))])
