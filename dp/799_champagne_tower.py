...
# top most glass is full, any excess liquid poured will fall equally to the
# glass immediately to the left and right of it.  When those glasses become
# full, any excess champagne will fall equally to the left and right of
# those glasses, and so on.  (A glass at the bottom row has it's excess
...
class Solution(object):
    def champagneTower(self, poured, query_row, query_glass):
        ...
        :rtype: float
        """
        dp1 = [None] * (query_row + 1)
        dp2 = [None] * (query_row + 1)
        curr_dp = dp2

        for r in range(query_row + 1):
            pre_dp = curr_dp
            curr_dp = dp1 if curr_dp == dp2 else dp2
            ...
            if r == 0:
                ...
                for i in range(r + 1):
                    curr_dp[i] = 0
                    if pre_dp[i] is not None:
                        curr_dp[i] += max(0, (pre_dp[i] - 1) / 2.0)
                    if i > 0:
                        curr_dp[i] += max(0, (pre_dp[i - 1] - 1) / 2.0)

        if curr_dp[query_glass] > 1:
            return 1
        elif curr_dp[query_glass] is None:
            return 0
        else:
            return curr_dp[query_glass]
