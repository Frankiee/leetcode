...
class Solution(object):
    def _count_mine(self, board, r, c):
        ret = 0
        for n_r, n_c in [(r + 1, c), (r - 1, c), (r, c + 1), (r, c - 1), (r + 1, c + 1),
                         (r + 1, c - 1), (r - 1, c - 1), (r - 1, c + 1)]:
            if 0 <= n_r < len(board) and 0 <= n_c < len(board[0]) and board[n_r][n_c] == 'M':
                ret += 1

        return ret

    def _dfs(self, board, r, c):
        if board[r][c] == 'M':
            board[r][c] = 'X'
        elif board[r][c] == 'E':
            mine_count = self._count_mine(board, r, c)
    ...
    def updateBoard(self, board, click):
        ...
        :rtype: List[List[str]]
        """
        self._dfs(board, click[0], click[1])
