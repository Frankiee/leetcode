...
# Every string in deadends and the string target will be a string of 4 digits from the 10,
# 000 possibilities '0000' to '9999'.
    ...
    def openLock(self, deadends, target):
        ...
        while to_do:
            ...
            while to_do:
                ...
                for i in range(4):
                    for c in [int(nxt[i]) + 1, int(nxt[i]) - 1]:
                        c = (c + 10) % 10
                        n_s = nxt[:i] + str(c) + nxt[i + 1:]
                        if n_s not in visited and n_s not in deadends:
                            nxt_to_do.append(n_s)
