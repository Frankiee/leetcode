...
class Solution(object):
    def _dfs(self, grid, r, c, current_gold):
        idx_gold = grid[r][c]
        grid[r][c] = 0
        current_gold += idx_gold

        for nxt_r, nxt_c in [(r + 1, c), (r - 1, c), (r, c + 1), (r, c - 1)]:
            if 0 <= nxt_r < len(grid) and 0 <= nxt_c < len(grid[0]) and grid[nxt_r][nxt_c] > 0:
                self._dfs(grid, nxt_r, nxt_c, current_gold)
