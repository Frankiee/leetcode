...
class Solution(object):
    def maxScore(self, cardPoints, k):
        ...
        sliding_window_size = total_cards - k

        if sliding_window_size <= 0:
            return sum(cardPoints)

        max_so_far = curr_window = sum(cardPoints[sliding_window_size:])
        for l in range(1, len(cardPoints) - sliding_window_size + 1):
            r = l + sliding_window_size - 1

            curr_window = curr_window + cardPoints[l - 1] - cardPoints[r]
            max_so_far = max(max_so_far, curr_window)
