...
# string "let", then there are 1 or more pairs of alternating variables and expressions,
...
# always two expressions e1, e2, and this expression evaluates to the addition of the evaluation
# of e1 and the evaluation of e2.
...
# we check from the innermost scope to the outermost in the context of the variable we are trying
# to evaluate.
...
# there is only a single space separating different components of the string, and no space
# between adjacent parentheses. The expression is guaranteed to be legal and evaluate to an integer.
# The length of expression is at most 2000. (It is also non-empty, as that would not be a legal
# expression.)
...
class Solution(object):
    def _evaluate_tokens(self):
        ...
        command = self.tokens.popleft()
        if command == 'let':
            while self.tokens:
                if self.tokens[0] == '(' or self.tokens[1] == ')':
                    value = self._evaluate_tokens()
                    self.tokens.popleft()
                    while stack:
                        self.variables[stack.pop()].pop()
                    return value
            ...
            if command == 'add':
                result = self._evaluate_tokens() + self._evaluate_tokens()
            else:
                result = self._evaluate_tokens() * self._evaluate_tokens()
    ...
    def evaluate(self, expression):
        ...
        :rtype: int
        """
        self.tokens = collections.deque(expression.replace('(', ' ( ').replace(')', ' ) ').split())
