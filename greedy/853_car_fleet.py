...
class Solution(object):
    def carFleet(self, target, position, speed):
        ...
        ret = 1
        prev_position, prev_speed = position_speed[0]
        prev_time = (target - prev_position) / float(prev_speed)

        for idx in range(1, len(position_speed)):
            curr_position, curr_speed = position_speed[idx]
            curr_time = (target - curr_position) / float(curr_speed)
