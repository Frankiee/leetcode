# https://leetcode.com/problems/flatten-a-multilevel-doubly-linked-list/
# 430. Flatten a Multilevel Doubly Linked List

# History:
# Google, Facebook
# 1.
# Mar 13, 2020
# 2.
# May 2, 2020

# You are given a doubly linked list which in addition to the next and previous pointers,
# it could have a child pointer, which may or may not point to a separate doubly linked list.
# These child lists may have one or more children of their own, and so on, to produce a
# multilevel data structure, as shown in the example below.
#
# Flatten the list so that all the nodes appear in a single-level, doubly linked list. You are
# given the head of the first level of the list.
#
#
#
# Example 1:
#
# Input: head = [1,2,3,4,5,6,null,null,null,7,8,9,10,null,null,11,12]
# Output: [1,2,3,7,8,11,12,9,10,4,5,6]
# Explanation:
#
# The multilevel linked list in the input is as follows:
#
#
#
# After flattening the multilevel linked list it becomes:
#
#
# Example 2:
#
# Input: head = [1,2,null,3]
# Output: [1,3,2]
# Explanation:
#
# The input multilevel linked list is as follows:
#
#   1---2---NULL
#   |
#   3---NULL
# Example 3:
#
# Input: head = []
# Output: []
#
#
# How multilevel linked list is represented in test case:
#
# We use the multilevel linked list from Example 1 above:
#
#  1---2---3---4---5---6--NULL
#          |
#          7---8---9---10--NULL
#              |
#              11--12--NULL
# The serialization of each level is as follows:
#
# [1,2,3,4,5,6,null]
# [7,8,9,10,null]
# [11,12,null]
# To serialize all levels together we will add nulls in each level to signify no node connects to
# the upper node of the previous level. The serialization becomes:
#
# [1,2,3,4,5,6,null]
# [null,null,7,8,9,10,null]
# [null,11,12,null]
# Merging the serialization of each level and removing trailing nulls we obtain:
#
# [1,2,3,4,5,6,null,null,null,7,8,9,10,null,null,11,12]
#
#
# Constraints:
#
# Number of Nodes will not exceed 1000.
# 1 <= Node.val <= 10^5


"""
# Definition for a Node.
class Node(object):
    def __init__(self, val, prev, next, child):
        self.val = val
        self.prev = prev
        self.next = next
        self.child = child
"""


class Solution1(object):
    def _flatten(self, head):
        if not head:
            return None, None

        rest_head, rest_end = self._flatten(head.next)

        if head.child:
            child_head, child_end = self._flatten(head.child)
            head.child = None

            head.next = child_head
            child_head.prev = head

            if rest_head:
                child_end.next = rest_head
                rest_head.prev = child_end

                return head, rest_end
            else:
                return head, child_end

        else:
            if rest_head:
                head.next = rest_head
                rest_head.prev = head

                return head, rest_end
            else:
                return head, head

    def flatten(self, head):
        """
        :type head: Node
        :rtype: Node
        """
        head, end = self._flatten(head)
        return head


"""
# Definition for a Node.
class Node(object):
    def __init__(self, val, prev, next, child):
        self.val = val
        self.prev = prev
        self.next = next
        self.child = child
"""


class Solution2(object):
    def _dfs(self, node):
        if not node:
            return None, None

        head = curr = node

        while not curr.child:
            if not curr.next:
                return head, curr
            curr = curr.next

        curr_next = curr.next

        child_head, child_end = self._dfs(curr.child)
        curr.child = None

        curr.next = child_head
        child_head.prev = curr

        if not curr_next:
            return head, child_end

        child_end.next = curr_next
        curr_next.prev = child_end

        _, rest_end = self._dfs(curr_next)

        return head, rest_end

    def flatten(self, head):
        """
        :type head: Node
        :rtype: Node
        """
        ret, _ = self._dfs(head)

        return ret
