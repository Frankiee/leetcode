...
# bike to that worker. (If there are multiple (worker, bike) pairs with the same shortest
# Manhattan distance, we choose the pair with the smallest worker index; if there are multiple
# ways to do that, we choose the pair with the smallest bike index). We repeat this process until
...
class Solution(object):
    ...
    def assignBikes(self, workers, bikes):
        ...
        :rtype: List[int]
        """
        ret = [None] * len(workers)

        workers_pqs = [[] for _ in range(len(workers))]

        for w_idx, w_pos in enumerate(workers):
            for b_idx, b_pos in enumerate(bikes):
                heappush(
                    workers_pqs[w_idx],
                    (self._manhattan_distance(b_pos, w_pos), w_idx, b_idx)
                )

        global_pq = []
        for w_pq in workers_pqs:
            heappush(global_pq, heappop(w_pq))

        used_bikes = set()
        while global_pq:
            _, w_idx, b_idx = heappop(global_pq)
            while b_idx in used_bikes:
                heappush(global_pq, heappop(workers_pqs[w_idx]))
                _, w_idx, b_idx = heappop(global_pq)
